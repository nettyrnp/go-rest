...
// ResetDB re-create the database schema and re-populate the initial data using the SQL statements in migration.sql.
...
func getSQLFile() string {
	if _, err := os.Stat("migrate/migration.sql"); err == nil {
		return "migrate/migration.sql"
	}
	return "../migrate/migration.sql"
}
